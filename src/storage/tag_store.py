...
class TagStore:
    ...
    async def create_tag(self, data: TagCreate) -> TagORM:
        ...
        async with self.session_factory() as session:
            ...
            tag = TagORM(
                ...
                created_by=data.created_by or self.default_actor,
                updated_by=data.created_by or self.default_actor,
                created_at=now,
                updated_at=now,
            )
            session.add(tag)
    ...
    async def get_tag_tree(self) -> list[TagTree]:
        ...
        stats = await self.get_tag_stats()
    ...
    async def list_tags(self, flat: bool = False) -> list[TagTree]:
        if flat:
            tags = await self.get_all_tags()
            stats = await self.get_tag_stats()
            count_map = {row["slug"]: row["count"] for row in stats}
        ...
        return await self.get_tag_tree()

    async def assign_tag(self, data: TagAssignmentCreate) -> TagAssignmentRead:
        async with self.session_factory() as session:
            tag = await self._resolve_tag(session, data.tag_id, data.tag_slug, data.tag_name)
            if not tag:
    ...
    async def get_target_bundle(self, target_type: str, target_ref: str, anchor: Optional[dict] = None) -> TagBundle:
        direct = await self._get_tags_for_target(target_type, target_ref, anchor=anchor)
        inherited: list[TagRead] = []
        aggregated = await self._get_aggregated_tags(target_type, target_ref)
        return TagBundle(direct_tags=direct, inherited_tags=inherited, aggregated_tags=aggregated)

    async def get_target_tag_names(self, target_type: str, target_ref: str) -> list[str]:
        bundle = await self.get_target_bundle(target_type, target_ref)
    ...
    async def get_email_tags(self, message_id: str) -> list[str]:
        return await self.get_target_tag_names(TARGET_TYPE_EMAIL_MESSAGE, message_id)
    ...
    async def _get_tags_for_target(self, target_type: str, target_ref: str, anchor: Optional[dict] = None) -> list[TagRead]:
        async with self.session_factory() as session:
            stmt = (
                select(TagORM)
    ...
    async def _get_aggregated_tags(self, target_type: str, target_ref: str) -> list[TagRead]:
        async with self.session_factory() as session:
    ...
    async def _resolve_tag(
        ...
        if tag_id is not None:
            return await session.get(TagORM, tag_id)
        if tag_slug:
            result = await session.execute(select(TagORM).where(TagORM.slug == tag_slug))
