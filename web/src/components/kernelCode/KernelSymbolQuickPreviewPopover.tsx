...
export default function KernelSymbolQuickPreviewPopover({
  ...
  return createPortal(
    <div
      ...
      className="fixed z-[70] flex flex-col overflow-hidden rounded-2xl border border-slate-700 bg-slate-900 shadow-[0_24px_80px_rgba(15,23,42,0.24)]"
      style={{
        left: frame.x,
        top: frame.y,
        width: frame.width,
    ...
    >
      <div
        className="flex cursor-move items-start justify-between gap-3 border-b border-slate-700 bg-gradient-to-r from-slate-800 to-slate-900 px-3 py-2"
        onDoubleClick={(event) => {
          const target = event.target as HTMLElement | null;
      ...
      >
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {symbol ? (
              <span className="rounded-full border border-sky-800 bg-sky-950 px-2 py-0.5 text-[11px] font-medium text-sky-300">
                {symbol}
              </span>
            ) : null}
          </div>
          <div className="mt-1 truncate text-xs font-mono text-slate-200">{candidate.path}</div>
          <div className="mt-1 flex flex-wrap items-center gap-2 text-[11px] text-slate-500">
            <span className="rounded-full bg-slate-700 px-2 py-0.5 text-slate-500">
              {candidate.local_file_available ? 'Local file' : 'External only'}
            </span>
            {containingSymbol ? (
              <span className="rounded-full border border-sky-800 bg-sky-950 px-2 py-0.5 text-sky-300">
                in {containingSymbol}
              </span>
            ) : null}
          </div>
        ...
        <div className="flex shrink-0 items-center gap-1">
          <button
            ...
            className="inline-flex h-8 w-8 items-center justify-center rounded-md text-sm font-semibold text-slate-500 transition hover:bg-slate-700 hover:text-slate-100"
            aria-label="Zoom out"
            title="Zoom out"
          >
            -
          ...
          <button
            type="button"
            onClick={() => setZoom(1)}
            className="inline-flex h-8 min-w-14 items-center justify-center rounded-md px-2 text-xs font-medium text-slate-500 transition hover:bg-slate-700 hover:text-slate-100"
          ...
          <button
            type="button"
            onClick={onOpenPage}
            className="inline-flex items-center justify-center rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-xs font-medium text-slate-200 transition hover:bg-slate-700"
          >
            Open page
          </button>
          <button
            type="button"
            onClick={onClose}
            className="inline-flex h-8 w-8 items-center justify-center rounded-md text-slate-500 transition hover:bg-slate-700 hover:text-slate-200"
      ...
      <div className="flex items-center justify-between gap-2 border-b border-slate-700 bg-slate-800 px-3 py-2 text-[11px] text-slate-500">
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          <span className="rounded-full bg-slate-900 px-2 py-0.5 text-slate-500">v{candidate.version}</span>
        ...
        <div className="flex items-center gap-2">
          <a
            ...
            className="inline-flex items-center gap-1 rounded-lg border border-slate-700 bg-slate-900 px-2.5 py-1.5 text-[11px] font-medium text-slate-200 transition hover:bg-slate-700"
          >
            Jump
          </a>
