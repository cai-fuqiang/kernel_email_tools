...
function highlightedTokenClass(token: string): string {
  ...
  if (C_KEYWORDS.has(token)) return 'font-medium text-sky-800';
...
export default function KernelSymbolQuickPreviewPopover({
  ...
  return createPortal(
    ...
    >
      <div
        className="flex cursor-move items-start justify-between gap-3 border-b border-slate-300 bg-gradient-to-r from-slate-50 to-white px-3 py-2"
        onDoubleClick={(event) => {
          const target = event.target as HTMLElement | null;
      ...
      >
        <div className="min-w-0">
          ...
          <div className="mt-1 truncate text-xs font-mono text-slate-900">{candidate.path}</div>
        ...
        <div className="flex shrink-0 items-center gap-1">
          <button
            ...
            className="inline-flex h-8 w-8 items-center justify-center rounded-md text-sm font-semibold text-slate-600 transition hover:bg-slate-200 hover:text-slate-950"
            aria-label="Zoom out"
            title="Zoom out"
          >
            -
          ...
          <button
            type="button"
            onClick={() => setZoom(1)}
            className="inline-flex h-8 min-w-14 items-center justify-center rounded-md px-2 text-xs font-medium text-slate-600 transition hover:bg-slate-200 hover:text-slate-950"
          ...
          <button
            type="button"
            onClick={onOpenPage}
            className="inline-flex items-center justify-center rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-medium text-slate-900 transition hover:bg-slate-200"
          >
            Open page
          </button>
          <button
      ...
      <div className="flex items-center justify-between gap-2 border-b border-slate-300 bg-slate-100 px-3 py-2 text-[11px] text-slate-600">
        ...
        <div className="flex items-center gap-2">
          <a
            ...
            className="inline-flex items-center gap-1 rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-[11px] font-medium text-slate-900 transition hover:bg-slate-200"
          >
            Jump
          </a>
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        <div
          ref={codeScrollRef}
          className="min-h-0 flex-1 overflow-y-scroll overscroll-contain bg-white p-3"
          style={{ scrollbarGutter: 'stable' }}
        >
          {!candidate.local_file_available ? (
            <div className="flex h-full items-center justify-center">
              <div className="max-w-lg rounded-xl border border-slate-200 bg-white px-4 py-4 text-sm leading-6 text-slate-900">
            ...
            <div className="rounded-xl border border-rose-500/30 bg-rose-500/10 px-4 py-3 text-sm text-rose-800">
              {error}
            </div>
          ) : fileLines.length > 0 ? (
            <div className="inline-block min-w-max rounded-xl border border-slate-200 bg-white shadow-lg">
              {fileLines.map((lineText, index) => {
                ...
                return (
                  <div
                    ...
                    className={`grid min-w-max grid-cols-[4.75rem_minmax(0,1fr)] border-b border-slate-200 ${
                      isTarget ? 'bg-amber-400/15' : 'hover:bg-slate-50'
                    }`}
                    style={zoomStyle}
                  >
                    <div className="select-none border-r border-slate-200 px-3 py-1.5 text-right font-mono text-slate-600">
                      {lineNum}
                    </div>
                    <pre className="overflow-x-auto px-3 py-1.5 font-mono text-slate-950">
