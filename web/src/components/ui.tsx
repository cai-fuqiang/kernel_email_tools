...
export function PageShell({
  ...
  return (
    <div className={cx('min-h-full bg-slate-950 px-4 py-4 md:px-6', className)}>
...
export function SectionPanel({
  ...
  return (
    <section className={cx('rounded-xl border border-slate-800 bg-slate-900 p-5', className)}>
...
export function SecondaryButton({
  ...
  return (
    <button
      {...props}
      className={cx(
        'inline-flex items-center justify-center gap-2 rounded-lg border border-slate-700 bg-slate-800 px-4 py-2 text-sm font-medium text-slate-200 transition hover:border-slate-600 hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-50',
        className,
      )}
    >
      {children}
...
export function IconButton({
  ...
  return (
    <button
      ...
      title={label}
      className={cx(
        'inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-700 bg-slate-800 text-slate-400 transition hover:border-slate-600 hover:bg-slate-700 hover:text-slate-200 disabled:cursor-not-allowed disabled:opacity-50',
        className,
      )}
    >
      {children}
...
export function InspectorSection({
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-slate-800 bg-slate-900/80">
      ...
      >
        <span className="flex min-w-0 items-center gap-2.5 text-sm font-semibold text-slate-100">
          <span className="text-slate-400">{icon}</span>
          <span className="truncate">{title}</span>
        </span>
        <span className="flex shrink-0 items-center gap-2">
          {headerExtra}
          <ChevronRight className={`h-4 w-4 text-slate-500 transition duration-200 ${collapsed ? '' : 'rotate-90'}`} />
        </span>
      </button>
      <div
...
export function SkeletonLine({ className = '' }: { className?: string }) {
  return <div className={cx('h-4 animate-pulse rounded bg-slate-700', className)} />;
}

export function SkeletonBlock({ className = '' }: { className?: string }) {
  return <div className={cx('animate-pulse rounded-lg bg-slate-700', className)} />;
}
