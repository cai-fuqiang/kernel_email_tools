...
| LLM 成本 | 见下方 Token 成本分析 |

### 1.5 Token 成本分析（全流程）

以 **100 万封邮件**为基准，DashScope qwen-plus 定价（输入 ~$0.5/M token，输出 ~$2/M token）：

| Phase | 范围 | 输入/条 | 输出/条 | 总 Token | 估算费用 |
|-------|------|---------|---------|----------|----------|
| P1 去噪&分类 | 全部 100 万封 | ~400 | ~100 | 5 亿 | **~$350** |
| P2 线程摘要 | 约 10 万个线程 | ~20,000 | ~500 | 20.5 亿 | **~$1,500** |
| P3 实体抽取 | P2 后有价值的线程(30%) | ~3,000 | ~500 | 1 亿 | **~$90** |
| **合计** | | | | | **~$2,000** |

这还没算 OpenAI GPT-4（贵 5~10 倍）。几百万封全量跑下来上万刀是可能的。

**省 Token 策略**：

1. **P1 用本地小模型**：分类任务很简单（只有 8 个类别 + 二分类 noise），用 80MB 的 `all-MiniLM-L6-v2` 做 embedding + 简单分类器，或者 7B 量化模型本地跑。零 API 费用。

2. **P1 只做去噪不分类**：noise 是二分类，token 更少。分类可以后续按需做。

3. **P2 不做全量，按需触发**：只在用户打开 ThreadDrawer 时生成摘要，缓存结果。1000 个活跃线程 ≈ $15。

4. **智能筛选**：优先给 patch version > 2（说明有持续讨论）、参与者 > 5 人、跨多个列表的线程做摘要。

5. **batch 处理**：批量发送 LLM 请求，多个邮件放一个 prompt 里分类，减少 system prompt 重复开销。

**建议起步策略**：

```
只开 P1 去噪（本地模型，零 API 费用）
  + P2 按需摘要（用户打开线程时触发，月费 <$50）
  等数据质量验证后 → P1 分类 + P3 实体抽取
```: AI 线程摘要（入库后）
