...
**后续范围**：
- Search result 的贡献度 chip 改成 hover preview。（2026-05-07：`ResultCard` 接入 compact inspector，hover 展示 tags / Knowledge / annotation / draft / score）
...
- Search result 从卡片内弹层升级为页面级 Floating Inspector Dock：结果卡只保留很小的 inspect 入口，右侧统一展示 tags、贡献度、score/source 和 tag 编辑；移动端使用底部 sheet。Search result 默认 snippet 改为 1 行，进一步贴近 ≤84px 高度验收。
...
- [ ] `npm run build` 通过；现有 e2e/功能：搜索、Ask 引用跳转、Draft 保存、批注审批、agent run 创建均不回归。（2026-05-07：前端 `npm run lint` / `npm run build` 多轮通过；功能回归待人工冒烟）
