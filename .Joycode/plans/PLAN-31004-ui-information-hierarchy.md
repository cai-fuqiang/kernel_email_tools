...
**后续范围**：
- Search result 保持快速扫描，不使用全局 floating dock；贡献度和 tag 数只以内联小 chip 展示。（2026-05-07：试验 `SearchInspectorDock` 后撤回，避免增加无效操作层）
...
- Search result 默认 snippet 改为 1 行，进一步贴近 ≤84px 高度验收；Search 页面不使用 floating dock，保留直接打开 thread 的扫描效率。
