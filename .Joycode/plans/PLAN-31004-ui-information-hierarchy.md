...
- [ ] 登录后 `/` 显示 Dashboard，5 秒内可识别"待办 + 主操作"。
- [ ] SearchPage / AskPage / KnowledgePage / ThreadDrawer 滚动 500px 仍可见 sticky 上下文条。
- [ ] SearchPage 单条结果默认垂直高度 ≤ 84px（当前 ~120px+）。
- [ ] KnowledgePage 拆分为 ≤ 5 个独立组件文件，主页面 ≤ 400 行。
- [ ] `npm run build` 通过；现有 e2e/功能：搜索、Ask 引用跳转、Draft 保存、批注审批、agent run 创建均不回归。
