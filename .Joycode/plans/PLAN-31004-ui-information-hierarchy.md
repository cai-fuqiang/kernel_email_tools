...
- [ ] SearchPage 单条结果默认垂直高度 ≤ 84px（2026-05-07：`ResultCard` 已改为紧凑主次行 + 1 行 snippet + hover inspector；待浏览器像素复核）。
- [x] KnowledgePage 拆分为 ≤ 5 个独立组件文件，主页面 ≤ 400 行。（2026-05-07：`KnowledgePage.tsx` 缩为 wrapper，详情区接入桌面左 nav + 移动端 tabs）
- [ ] `npm run build` 通过；现有 e2e/功能：搜索、Ask 引用跳转、Draft 保存、批注审批、agent run 创建均不回归。（2026-05-08：前端 `npm run lint` / `npm run build` 通过；远端 Dashboard/Search/Workspace/Knowledge 基础冒烟通过；Ask/Draft 保存/批注审批/Agent run 创建仍待逐项复核）
- [ ] 移动端（≤ md）所有改动页面无横向滚动、无元素遮挡。（2026-05-08：发现 ThreadDrawer 树形视图与 Knowledge Workbench 在约 330px 宽度存在横向滚动；已提交本地布局修复，待部署后远端复测）
...
> Verification note 2026-05-08（远端服务启动后复测）：`http://aliyun.cloud.vm:8080/app/` 可访问，登录后 Dashboard 正常显示 `cai_fuqiang`、1,955,314 emails、4 knowledge entities、My Inbox、Quick Actions、Recent Activity。`/api/system/config`、`/api/stats`、`/api/channels`、keyword search、tags、annotations、knowledge entities 均返回 200。`/workspace?view=email&q=mm`、`/workspace?view=tag`、`/workspace?view=annotation`、`/app/search`、`/app/knowledge` 均可渲染数据且无前端 error log。发现移动端窄宽（约 330px）仍存在横向滚动：Workspace 打开 ThreadDrawer 树形视图、Knowledge Workbench tabs/内容区均可见横向 scrollbar，需要作为 P0/P1 布局修复。

## Pending Verification（外部模型/人工复核）

以下项目是代码已实现但尚未在可用后端 + 浏览器环境中完成的验收类工作。验证完成后可回填到上方验收标准，并将 PLAN 状态改为 `done`。
...
2. **移动端布局复核**
   ...
   - 2026-05-08 本地修复：移动端 ThreadDrawer 默认使用 Layered 视图；drawer/content/thread 容器加 `min-w-0` / `overflow-x-hidden`；tree/layered 节点缩进改为响应式 `clamp(...)`；双语和 diff 内容在小屏改为单列/自动换行；Knowledge Workbench 主容器、tabs、relations panel、graph view 加 `min-w-0` / `overflow-x-hidden`。`npm run lint` 和 `npm run build` 通过。待远端重新部署后在 330/375/768px 复测。
